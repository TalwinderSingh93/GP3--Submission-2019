...
//This method is called by rigid body to convert the btTransform to Transform
void Physics::ConvertbtTransformToTransform(btTransform bt, Transform* t)
{
	t->SetRotation(glm::quat(bt.getRotation().getW(), bt.getRotation().getX(), bt.getRotation().getY(), bt.getRotation().getZ()));
	t->SetPosition(glm::vec3(bt.getOrigin().getX(), bt.getOrigin().getY(), bt.getOrigin().getZ()));
}
